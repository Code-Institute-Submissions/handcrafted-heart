...
class CaseInsensitiveAuth:
    """
    Authentication of a User by using a case-insensitive query to check a
    combination of the supplied email/username and password.
    To avoid the risk of having two users with similar usernames,
    distinguished only by letter case (e.g. 'steve' and 'Steve'), make sure to
    update my User model to save usernames as lower case entries to
    the database. This will ensure all usernames have unique spellings, and as a result,
    the case insensitive query will return a single result only.
    """
    def authenticate(self, username_or_email=None, password=None):
        ...
        # Then get the first result of the query (which is the user).
        user = users[0]
